function virtualenv_prompt_info(){
  ...
  echo "${ZSH_THEME_VIRTUALENV_PREFIX:=[}${VIRTUAL_ENV:t}"
}
      ...
      if [[ "$ENV_NAME" != "" ]]; then
        ...
        if [[ "$VIRTUAL_ENV" != "$WORKON_HOME/$ENV_NAME" ]]; then
          if [[ -e "$WORKON_HOME/$ENV_NAME/bin/activate" ]]; then
            ...
            # we've left a directory with a .venv or similar AND are in a git
            # repo, but that repo does not have an associated virtualenv
